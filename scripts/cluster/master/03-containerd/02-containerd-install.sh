...
sudo wget https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.33.0/crictl-v1.33.0-linux-amd64.tar.gz
...
sudo cp /root/kubernetes-binary-install/configs/containerd/containerd.tolm /etc/containerd/certs.d/docker.io/hosts.toml

#Configure runtime and image pulling to use containerd
sudo cp /root/kubernetes-binary-install/configs/containerd/crictl.yaml /etc/crictl.yaml
