...
sudo cp /root/kubernetes-binary-install/configs/modules-load.conf /etc/modules-load.d/k8s.conf
sudo systemctl restart systemd-modules-load.service

#Modify Linux Kernel Parameters (enable bridge traffic forwarding)
sudo cp /root/kubernetes-binary-install/configs/network.conf /etc/sysctl.d/k8s.conf
...
sudo cp /root/kubernetes-binary-install/configs/ipvs.conf /etc/sysconfig/modules/ipvs.modules
